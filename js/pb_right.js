...
!function(){
	...
	var mylogined = document.getElementById('mylogin');
	...
	for(var i=0; i<p_contro.length; i++){
		...
		// p_contro[i].onmouseenter = function(){

		// 	clearInterval(dkTimer);
		// }
		// p_contro[i].onmouseleave = function(){

		// 	dispear();
		// }
		p_contro[i].onclick = function(ev)
		{

			for(var j=0;j<p_contro.length;j++)
			{
				p_contro[j].style.transform = 'translate(100%)';
			}
			//if(p_contro[i]!=p_contro[2]){
				this.style.transform = 'translate(0%)';
				p_contro[2].style.transform = '';
			//}
			ev.cancelBubble = true;
		}
	}
	...
	//模块儿相加
	//console.log(abt_content);
	abt_content.innerHTML = news+mission+knowledge+inspector+flowManage+fastStart;
	

	//右键操作
	
	var abt_con_item = document.getElementsByClassName('abt_con_item');
	var rightShow = document.getElementById('rightShow');
	var abt_content = document.getElementById('abt_content');
	
		abt_content.oncontextmenu = function(ev){
			rightShow.style.top = ev.clientY +'px';
			rightShow.style.left = ev.clientX +'px';
